...
def RecordW(use, function, time):
    ...
    :return:
    '''
    with open('./qqai2/plugins/data/reply.json','r') as recordR:       #向reply.json文件存入信息
        data = json.load(recordR)
        data['name'].append(use)
        data['function'].append(function)
        data['time'].append(time)
    with open('./qqai2/plugins/data/reply.json', 'w') as recordW:
        json.dump(data, recordW, indent=4, ensure_ascii=False)
def RecordR():                                                          #读取reply.json
    with open('./qqai2/plugins/data/reply.json','r') as recordR:
