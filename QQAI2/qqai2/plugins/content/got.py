        ...
        if RecordR()['time'][RecordR()['name'].index(user)]+60 > time.time():
            ...
            # 商品购买
                ...
                if isinstance(content,list) and len(content) == 2:
                    ...
                    RecordV(user)
                    return {'str':'参数错误≧ ﹏ ≦'}
            elif RecordR()['function'][indexes] == '气象预报':
                RecordV(user)
                return {'str':Weather(AnalysisNews(data)).Forecast()}
            elif RecordR()['function'][indexes] == '气象实况':
                print(data)
                RecordV(user)
                return {'str':Weather(AnalysisNews(data)).now()}
